...
if __name__ == '__main__':
    ...
    while len_count < len(lines):
        ...
        for i,l in enumerate(lines[len_count:len_count+1000]):
            ...
            # BGR channel interchange and flipping row and columns
            img[0,:,:] = imgl[2,:,:].T
            img[1,:,:] = imgl[1,:,:].T
            img[2,:,:] = imgl[0,:,:].T
            X[i] = img
    ...
    if len_count > len(lines):
        len_count = len_count - 1000
        X = np.zeros((len(lines)-len_count,3,SIZE_C,SIZE_R),dtype='f4')
